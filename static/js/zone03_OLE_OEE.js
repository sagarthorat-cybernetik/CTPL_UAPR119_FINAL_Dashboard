...
async function loadPage(page = 1) {
  ...
  try {
    const res = await fetch("/fetch_data_zone03_ole_oee", {
      method: "POST",
...
function renderTable(data, columns) {
  const table = document.getElementById("dataTable");
    ...
    columns.forEach(col => {
       ...
       if (col.includes("DateTime") || col.includes("Shift_User") || col.includes("OperationalShift")) {
            th.textContent = col;
        } else {
            th.textContent = `${col}%`;
        }
...
async function startExport() {
  ...
  try {
    const res = await fetch("/export_excel_zone03_ole_oee", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
// Navigate to zone02 ole oee (kept)
function zone01oleoee() {
  window.location = "/zone01_ole_oee";
}

// Navigate to zone01 ole oee dashboard (kept)
function zone02oleoee() {
  window.location = "/zone02_ole_oee";
}
// Navigate to zone01 ole oee dashboard (kept)
function zone03oleoee() {
  window.location = "/zone03_ole_oee";
}